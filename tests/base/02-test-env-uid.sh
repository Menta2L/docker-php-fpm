...
if ! docker_exec "${did}" "id | grep 'uid=1005'" "--user=sandbox"; then
	docker_logs "${did}" || true
	docker_stop "${did}" || true
	echo "Failed"
	exit 1
...
if ! docker_exec "${did}" "id | grep 'uid=1000'" "--user=sandbox"; then
	docker_logs "${did}" || true
	docker_stop "${did}" || true
	echo "Failed"
	exit 1
...
if ! docker_exec "${did}" "id | grep 'uid=33'" "--user=sandbox"; then
	docker_logs "${did}" || true
	docker_stop "${did}" || true
	echo "Failed"
	exit 1
